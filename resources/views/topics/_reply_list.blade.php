    ...
    <li class=" media" name="reply{{ $reply->id }}" id="reply{{ $reply->id }}">
      <div class="media-left">
        <a href="{{ route('users.show', [$reply->user_id]) }}">
          <img class="media-object img-thumbnail mr-3" alt="{{ $reply->user ? $reply->user->name : '该用户已被删除' }}" src="{{ $reply->user ? $reply->user->avatar : '' }}" style="width:48px;height:48px;" />
        </a>
      </div>

      <div class="media-body">
        <div class="media-heading mt-0 mb-1 text-secondary">
          <a href="{{ route('users.show', [$reply->user_id]) }}" title="{{ $reply->user ? $reply->user->name : '该用户已被删除' }}">
            {{ $reply->user ? $reply->user->name : '该用户已被删除' }}
          </a>
